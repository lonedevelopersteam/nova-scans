...
<head>
    ...
    <title>Manage Admin - Cosmic Scan</title>
...
@section('content')
    <div class="container mt-4">
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                <h5 class="card-title" id="adminCardTitle">Total Admin Terdaftar: <span id="totalAdminCount">Loading...</span></h5>
    ...
    <button type="button" class="btn btn-primary rounded-circle shadow-lg fab" id="addAdminButton">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="text-center">
                        ...
                        <p><strong>Hanya Admin yang dapat mengelola data admin.</strong></p>
                    </div>
                </div>
    ...
    <script>
        ...
        function updateTotalAdminCount(total) {
            const totalCountElement = document.getElementById('totalAdminCount');
        ...
        async function searchAdminData(query) {
            ...
            adminTableBody.innerHTML = '<tr><td colspan="5" class="text-center">Searching...</td></tr>';
            isSearchMode = true;
            currentSearchQuery = query;

            try {
                const response = await fetch(`${searchApiUrl}?q=${encodeURIComponent(query)}&role=Admin`, {
                    method: 'GET',
                    headers: {
                        'Accept': 'application/json',
                ...
                if (response.ok && data.success) {
                    ...
                    updateTotalAdminCount(data.data.data.length);
                } else {
                    throw new Error(data.message || 'Failed to search data');
                }
            } catch (error) {
                console.error('Search Error:', error);
                adminTableBody.innerHTML = `<tr><td colspan="5" class="text-center text-danger">Error: ${error.message}</td></tr>`;
                paginationContainer.innerHTML = '';
                updateTotalAdminCount(0);
            }
        }

        async function fetchAdminData(page = 1) {
            adminTableBody.innerHTML = '<tr><td colspan="5" class="text-center">Loading...</td></tr>';

            try {
                const response = await fetch(`${apiUrl}?role=Admin&page=${page}`, {
                    method: 'GET',
                    headers: {
                        'Accept': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content
                ...
                if (response.ok && data.success) {
                    ...
                    updateTotalAdminCount(data.data.pagination.total);
                } else {
                    throw new Error(data.message || 'Failed to load data');
                }
            } catch (error) {
                console.error('Error:', error);
                adminTableBody.innerHTML = `<tr><td colspan="5" class="text-center text-danger">${error.message}</td></tr>`;
                paginationContainer.innerHTML = '';
                updateTotalAdminCount(0);
            }
        }
        ...
        function renderTable(users, isSearch = false) {
            ...
            if (users.length === 0) {
                const message = isSearch ? 'Tidak ada hasil pencarian.' : 'Tidak ada data admin.';
                adminTableBody.innerHTML = `<tr><td colspan="5" class="text-center">${message}</td></tr>`;
                return;
            }
            ...
            users.forEach((user, index) => {
                ...
                const deleteButtonTitle = !isAdmin ? 'Hanya Admin yang dapat menghapus' :
        ...
        searchInput.addEventListener('keypress', (e) => {
            if (e.key === 'Enter') {
                searchAdminData(searchInput.value.trim());
            }
        });
        ...
        if (addAdminForm) {
            addAdminForm.addEventListener('submit', async function(event) {
                ...
                try {
                    ...
                    form.reset();
                    if (isSearchMode) {
                        searchAdminData(currentSearchQuery);
                    } else {
                        fetchAdminData(currentPage);
                    }
        ...
        function updateUIBasedOnRole() {
            ...
            if (!isAdmin) {
                ...
                addAdminButton.title = 'Hanya Admin yang dapat menambah admin baru';
                addAdminButton.style.opacity = '0.6';
                addAdminButton.style.cursor = 'not-allowed';
            } else {
                addAdminButton.disabled = false;
                addAdminButton.title = 'Tambah Admin Baru';
                addAdminButton.style.opacity = '1';
                addAdminButton.style.cursor = 'pointer';
            }
